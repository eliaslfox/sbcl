...
;;  (:translate symbol-function)
  (:generator 2
    (inst mov function (ldb (byte symbol-function-bits 0) -1))
...
;;  (:translate symbol-function)
  ...
  (:generator 4
    ...
    (inst and :qword function (object-slot-ea symbol symbol-func-slot other-pointer-lowtag))
    ;; TODO: restarts?
    (inst jmp :z (generate-error-code vop 'undefined-fun-error symbol))))
...
(define-vop (fdefn-fun)
  ...
  (:policy :fast-safe)
  ;; TODO: we want 2 variants of this: one which does no translation of the field
  ;; (returning 0 if 0) and one which converts 0 to NIL since most uses of FDEFN-FUN
  ;; expect the empty value to be NIL.
  (:translate fdefn-fun)
  (:generator 2
    (inst mov null nil-value)
    (inst mov result (ldb (byte symbol-function-bits 0) -1))
...
(define-vop (set-direct-callable-fdefn-fun)
  ...
  (:generator 38
    ...
    (pseudo-atomic ()
      (inst mov temp #.(mask-field (byte (- 64 symbol-function-bits) symbol-function-bits) -1))
      ;; read the linker index
      (inst and :qword temp (object-slot-ea fdefn fdefn-fun-slot other-pointer-lowtag))
      ;; merge in the new bits
      (inst or temp function)
      (storew temp fdefn fdefn-fun-slot other-pointer-lowtag)
      #-linker-space (storew raw-word fdefn fdefn-raw-addr-slot other-pointer-lowtag))))
