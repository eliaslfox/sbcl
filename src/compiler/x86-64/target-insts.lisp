...
(define-symbol-macro lisp-linkage-table-base
    (sb-alien:extern-alien "lisp_linkage_table" sb-alien:unsigned))
(defun linkage-addr->name (code value mode)
  (let ((index (ash (ecase mode
                      (:address (- value lisp-linkage-table-base))
                      (:index value))
                    (- sb-vm:word-shift))))
    ;; technically can stop scanning after code-n-fdefns probably
    (loop for i from sb-vm:code-constants-offset below (code-header-words code)
          do (let ((obj (code-header-ref code i)))
               (when (and (typep obj '(or (and symbol (not null)) fdefn))
                          (eql (sb-c::fname-linker-index obj) index))
                 (return-from linkage-addr->name
                   (if (fdefn-p obj) (fdefn-name obj) obj)))))
    ;; these won't be found in the code header
    (dovector (sym sb-vm:+static-fdefns+)
      (when (eql (sb-c::fname-linker-index sym) index)
        (return-from linkage-addr->name sym)))))
  ...
  (cond ((typep value 'machine-ea)
         ...
         (when (and code
           ...
           (let ((name
             ...
             (when name
               ;; #\' (and not "#'")  implies calling via a function name
          ...
          (machine-ea
           (let ((linkage ; gets cleared by PRINT-MEM-REF
             ...
             (cond ((eq (machine-ea-base value) :rip)
                    (+ (dstate-next-addr dstate) (machine-ea-disp value)))
                   (linkage
                    (+ (machine-ea-disp value) lisp-linkage-table-base)))))
