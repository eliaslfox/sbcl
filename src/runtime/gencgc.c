...
{
    int index;
    index = (widetag == FDEFN_WIDETAG)
            ? extract_fname_index(((struct fdefn*)fname)->fun) :
              extract_fname_index(((struct symbol*)fname)->func);
    // If this object consumed an index in the linkage table, then free the table index
    if (index) {
...
static lispobj* range_dirty_p(lispobj* where, lispobj* limit, generation_index_t gen)
{
    ...
    for ( ; where < limit ; where += nwords ) {
        ...
        if (widetag == INSTANCE_WIDETAG) {
            ...
            if (!ptr_ok_to_writeprotect(decode_fname_taggedfun(s->func), gen)) return where;
            if (!ptr_ok_to_writeprotect(entrypoint_taggedptr(lookup_fname_entrypoint(s->func)),
                                        gen)) return where;
            ...
            if (!ptr_ok_to_writeprotect(decode_fname_taggedfun(f->fun), gen)) return where;
            if (!ptr_ok_to_writeprotect(entrypoint_taggedptr(lookup_fname_entrypoint(f->fun)),
                                        gen)) return where;
        }
#endif
