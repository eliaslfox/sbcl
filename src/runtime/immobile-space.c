                             ...
                             int gen, int keep_gen, int new_gen)
{
  ...
  case SYMBOL_WIDETAG:
    ...
    if (younger_p(decode_fname_taggedfun(sym->func), gen, keep_gen, new_gen)) return 1;
    if (younger_p(entrypoint_taggedptr(lookup_fname_entrypoint(sym->func)),
...
static void process_fname_fun_slot(lispobj* slot, lispobj* obj)
{
    lispobj word = *slot;
    // Process the function linker table entry
    int index = extract_fname_index(word);
    if (index) {
        char* entrypoint = (char*)lisp_linkage_table[index];
        if (!entrypoint)
            fprintf(stderr, "WARNING: function table index %d is zero (obj=%p)\n", index, obj);
        else
            lisp_linkage_table[index] = compute_new_raw_entrypoint(entrypoint);
    }
...
static void fixup_space(lispobj* where, size_t n_words)
{
    ...
    while (where < end) {
        ...
        case CLOSURE_WIDETAG:
          ...
          break;
        case FDEFN_WIDETAG:
          adjust_words(where+1, 1); // name
          struct fdefn *fdefn = (void*)where;
          process_fname_fun_slot(&fdefn->fun, where);
          break;
        case SYMBOL_WIDETAG:
          ...
          process_fname_fun_slot(&s->func, where);
#else
